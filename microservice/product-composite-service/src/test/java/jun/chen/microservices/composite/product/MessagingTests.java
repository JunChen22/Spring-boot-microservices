...
import static jun.chen.microservices.composite.product.IsSameEvent.sameEventExceptCreatedAt;
import static org.hamcrest.MatcherAssert.assertThat;
import static org.hamcrest.Matchers.is;
import static org.junit.jupiter.api.Assertions.assertEquals;
import static org.springframework.http.HttpStatus.ACCEPTED;
import jun.chen.api.composite.product.ProductAggregate;
import jun.chen.api.composite.product.RecommendationSummary;
import jun.chen.api.composite.product.ReviewSummary;
...
public class MessagingTests {
    ...
    void createCompositeProduct2() {
        ...
        final List<String> productMessages = getMessages("products");
        final List<String> recommendationMessages = getMessages("recommendations");
        final List<String> reviewMessage = getMessages("reviews");

        // Assert one create product event queue up
        assertEquals(1, productMessages.size());
        ...
        assertEquals(1, reviewMessage.size());
        ...
        assertThat(reviewMessage.get(0), is(sameEventExceptCreatedAt(expectedReviewEvent)));
    }
    ...
    private Message<byte[]> getMessage(String bindingName) {
        try {
            ...
            // If the message queue member variable in the target object contains no queue when the receive method
            // is called, it will cause a NPE to be thrown. So we catch the NPE here and return null to indicate that
            // no messages were found
            LOG.error("getMessage() received a null pointer exception with binding = {}", bindingName);
            return null;
        }
    }
